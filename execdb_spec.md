...
* **バージョン不一致の扱い:** 取り込み元ファイルのフッター（`Version`フィールド、§7参照）が、今動いているExecDBのバージョンと異なる場合、警告を表示した上で処理を続行する（拒否はしない）。
...
```go
import "github.com/xxx/execdb/engine"
...
// 永続化（別ファイルへ明示的にスナップショットを書き出す）
db.Snapshot()

// 永続化（自分自身の実行ファイルを上書きする）
db.Overwrite()

...
│   └── tour/                     ← ExecDB入門（体験型ガイド、順を追って学ぶ形式）
├── .devcontainer/
├── .vscode/
├── .claude/
│   ├── settings.json
...
- **docs/**: 利用者向けドキュメント一式（フェーズ④完了後、Step 9/10で新設。
  ...
  - **docs/tour/**: 実装が完了してから作成する入門ガイド（体験型、順を追って
    学ぶ形式。Step 10で新設）。`README.md`（索引）＋`01`〜`04`の番号付き
    章立てで、1つ前の章を前提に次の章へ進む構成——`docs/usage/`（索引的な
    リファレンス）・`docs/examples/`（1本ずつ自己完結したユースケース）とは
    読む順序が意味を持つ点が異なる。各章のコマンド・出力例は、他の`docs/`

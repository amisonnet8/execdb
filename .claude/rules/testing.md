...
## 手動デバッグ中に残ったバックグラウンドサーバーの孤児プロセスが`tests/e2e.sh`を偽装する（フェーズ④Step 5で発覚）

`tests/e2e.sh`自体の孤児プロセス処理（`SERVER_PIDS`配列＋
`trap cleanup EXIT`）は正しく機能しているが、**`make test`を経由せず
手動で`./execdb -n -p ... &`のようにバックグラウンドサーバーを起動して
実機確認する際、作業後に`kill`し忘れると、次回の`tests/e2e.sh`実行が
サイレントに壊れる。** `start_server`はバックグラウンド起動の成否
（`net.Listen`が実際に成功したか）を確認せず、`wait_for_tcp`も「その
ポートに“誰か”が listen していれば」成功と判定するため、孤児プロセスが
先に同じポートを掴んでいると、**新しく起動したはずの（実際にはbindに
失敗して即終了した）サーバーではなく、古い孤児プロセスに対してテストが
実行されてしまう**。孤児側は前回までのデータが残ったままなので、
`checkTransactionIsolation`のような「固定値を1回だけINSERTして件数を
確認する」類のテストが、蓄積した重複行のせいで謎の失敗をする——
`tests/e2e.sh`側のコード自体は正しいため、`FAIL:`メッセージも出ないまま
（`go run`が別のエラーで失敗するか、無関係な位置で`exit 1`になる）原因の
見当がつきにくい。

**対処:** 手動でバックグラウンドサーバーを起動して調査したら、作業の
最後に必ず`kill`する。`tests/e2e.sh`の実行が原因不明に失敗する・
`ok - ...`の途中で唐突に止まる場合は、まず
